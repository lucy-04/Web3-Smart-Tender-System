...
const TenderLogin = () => {
    ...
    const isStep1Valid =
        ...
        (formData.litigation_status !== "active" ||
            formData.litigation_details?.trim());

    const isStep2Valid =
        requiredStep2.every((field) => metricsData[field]?.toString().trim());
    ...
    return (
        ...
        opacity-30 blur-[160px] rounded-full animate-aurora" />

                <div className="absolute bottom-0 right-0 w-[700px] h-[400px] 
        bg-gradient-to-r from-purple-500 via-blue-600 to-cyan-400 
        opacity-20 blur-[150px] rounded-full animate-aurora-slow" />
            </div>
            ...
            <div className="max-w-6xl mx-auto px-12 py-16">
                ...
                <div className="mb-14">
                    <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                        <div
                            className={`h-full bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 transition-all duration-500 ${step === 1 ? "w-1/2" : "w-full"
                                }`}
                        />
                    </div>
                </div>
                ...
                {step === 1 && (
                    <Card title="Compliance & Qualification">
                        <Grid>
                            ...
                            <FileInput
                                name="labour_license"
                                label="Labour License (Upload Certificate PDF)"
                                onChange={handleChange}
                            />
                ...
                {step === 2 && (
                    <Card title="AI Performance Metrics">
                        ...
                        <div className="flex justify-between mt-14">
                            ...
                            <PrimaryButton disabled={!isStep2Valid}>
                                Submit Application
                            </PrimaryButton>
                        </div>
...
const Section = ({ title }) => (
    <h3 className="text-xl font-semibold mt-12 mb-6 text-white">
        {title}
    </h3>
);
...
const SecondaryButton = ({ children, onClick }) => (
    <button
        onClick={onClick}
        className="px-10 py-3 bg-white/10 hover:bg-white/20 rounded-xl transition"
    >
        {children}
    </button>
);
